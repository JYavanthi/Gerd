...
@Component({
  ...
  templateUrl: './comorbidities3.component.html',
  styleUrls: ['./comorbidities3.component.css']
})
export class Comorbidities3Component {
  tabId = 1;
...
onNext() {
   this.router.navigate([`followUpTwo/assessment/${this.patientService.getPatientId()}`], {
    state: {
      tabId: this.tabId,
...
getStatusClass(step: number): string {
  ...
  back() {
  this.router.navigate([`/followUpTwo/chiefComplaint/${this.patientService.getPatientId()}`], {
    state: {
      tabId: this.tabId,
      patientId: this.patientService.getPatientId(),
      isViewMode: this.isViewMode
