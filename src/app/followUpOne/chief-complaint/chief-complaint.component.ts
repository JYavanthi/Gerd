...
export class ChiefComplaintComponent2 implements OnInit {
  tabId = 2;
  patientId: any;
 ...
 ngOnInit(): void {
  ...
  if (this.stage === 0 || this.stage === 1) {
    this.activeTabId = 2; // Follow-up 1 becomes active after baseline is green
  } else if (this.stage === 3) {
    this.activeTabId = 3; // Follow-up 2 becomes active
  } else {
    this.activeTabId = 1; // Default to Baseline
  }

  console.log('Active tab on load:', this.activeTabId);
...
console.log("Chief View mode",this.isViewMode)
...
back() {
  ...
  console.log("Tabid", this.tabId)
  this.router.navigate([`followUpOne/comorbidities/${this.patientId}`], {
    state: {
      data: this.formData,
      isViewMode: this.isViewMode,
      patientId: this.patientId,
