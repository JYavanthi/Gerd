...
export class AssessmentComponent2 {
...
onNext() {
  ...
  if (currentUrl.includes('follow-up-1')) {
    ...
    OnNext(){
    this.router.navigate([`/followUpOne/management/${this.patientService.getPatientId()}`], {
    state: {
      tabId: this.tabId,
...
goback(){
  ...
  back() {
  this.router.navigate([`followUpOne/comorbidities/${this.patientService.getPatientId()}`], {
    state: {
      tabId: this.tabId,
      patientId: this.patientId,
      isViewMode: this.isViewMode
